  ...
  <div class="MobileInfoLogin">
    ...
    <div class="content-box">
      <group>
        ...
        <x-input v-model="sign" class="mobile" title="验证码" placeholder="请输入验证码">
          <span class="get-verification-code" v-if="state" slot="right">{{time}}秒后重新获取</span>
          <span @click="resetSign" class="get-verification-code-btn" v-else slot="right">重新获取</span>
        </x-input>
        <x-button
  ...
  methods: {
    async getSign() {
      let data = await util.getData({
        url: `/ajax/sendssms/?regs-${this.mobile}.html`,
        method: "get"
      });
      this.$vux.toast.show({
        text: data,
    ...
    async login() {
      let data = await util.getData({
        url: `/users/login?login_username=${this.$route.query.mobile}&login_type=1&mobile_code=${this.sign}`,
        method: "get",
      });
